...
B_ROUTE_PW = os.getenv("B_ROUTE_PW")
...
class CommEchoNet:
    ...
    def SetID(self, szID, szPassword):
        ...
        if bResult:
            ...
            if (nPwLen > 0) and (nPwLen <= 32):
                ...
                bResult = self.SendCommand(
                    "SKSETPWD " + format(nPwLen, "x") + " " + szPassword
                )
        return bResult
        ...
        if bResult:
            ...
            while not bConnected:
                ...
                if szData.startswith("EVENT 22"):
                    ...
                    if ":" in szData:
                        # stripで前後の空白を削除
                        self.dictScanedDesc.update(
                            dict([[x.strip() for x in szData.split(":")]])
                        )
                    else:
                        bDescription = False
    ...
    def SendEchonetCommand(self, szEPC):
        ...
        szCommand = "SKSENDTO 1 {0} 0E1A 1 0 {1:04X} ".format(
            self.LocalIPAddr, len(szEchonetCommand)
        )
        ...
        while (not bConnected) and (fTimeOut > time.time()):
            ...
            if szData.startswith("ERXUDP"):
                ...
                seoj = szEData[
                    8 : 8 + 6
                ]  # 8bytes目から6bytesがSEOJ(送信元ECHONET Liteオブジェクト)
                deoj = szEData[
                    14 : 14 + 6
                ]  # 14bytes目から6bytesがDEOJ(受信先ECHONET Liteオブジェクト)
                esv = szEData[
                    20 : 20 + 2
                ]  # 20bytes目から2bytesがESV(ECHONET Liteサービス結果)
                epc = szEData[
                    24 : 24 + 2
                ]  # 24bytes目から2bytesがEPC(ECHONET Liteプロパティ)
                pdc = int(
                    szEData[26 : 26 + 2], 16
                )  # 26bytes目から2bytesがPDC(EDTのバイト数)
                # 正常結果か?
                if (esv == "72") and (pdc > 0):
                    # 結果を取得する
                    nResult = int(
                        szEData[28 : 28 + (pdc * 2)], 16
                    )  # 28bytes目から(PDC*2)bytesがEDT(結果の数値)
                    bConnected = True
...
def main(arg1, arg2):
    ...
    logging.info("瞬時電力計測値:{0}[W]".format(nMeasuredPower))
    fIntegratedpower = echonet.GetIntegratedpower()
    logging.info("積算電力量計測値:{0}[KW]".format(fIntegratedpower))
    boxled.off(4)
    # POST HomeAssistant
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "integratedpower": fIntegratedpower,
                },
